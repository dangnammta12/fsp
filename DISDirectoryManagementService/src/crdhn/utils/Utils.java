...
public class Utils {
    ...
    public static String genKeyItem(long itemId, int type) {
        try {
            String key = itemId + "_" + type + "_FSP";
            MessageDigest digest256 = MessageDigest.getInstance("SHA-256");
            return Utils.toHex(key.getBytes(), digest256);
        } catch (NoSuchAlgorithmException e) {
            throw new RuntimeException("SHA-256 is not available", e);
        }
    }

    public static long getParentFolderFromPath(String path) {
...
//                    .substring(1));
    ...
    public static String listLongToStringReverse(List<Long> listId) {
        String result = "";
