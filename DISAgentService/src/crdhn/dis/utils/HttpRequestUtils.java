...
// Create cookies for first and last names.      
    ...
    public static String buildParamRequestString(JSONObject mapParam) throws UnsupportedEncodingException {
        String str = "";
        Iterator iter = mapParam.keys();
        while (iter.hasNext()) {
            try {
                String key = (String) iter.next();
                if (mapParam.has(key)) {
                    String value = URLEncoder.encode(mapParam.getString(key), "UTF-8");
                    if (str.equals("")) {
                        str += key + "=" + value;
                    } else {
                        str += "&" + key + "=" + value;
    ...
    public static String sendHttpRequest(String requestUrl, String method, JSONObject params) throws IOException {
//        String requestParam = buildParamRequestString(params);
        try {
            params.put("agentKey", Configuration.agentKey);
            String data_encrypt = AsymmetricCryptography.getInstance().buildParamsEncryptData(params.toString(), Configuration.publicKeyDIS);
            if (data_encrypt == null) {
                return "{\"error_code\":-1,\"error_message\":\"data_encrypt=" + data_encrypt + "\"}";
            }
            ...
            if ("POST".equals(method)) {
                ...
                newRequestUrl = requestUrl + "?" + data_encrypt;
...
//        urlConn.setRequestProperty("Content-Type", "multipart/form-data;");
            if (params.has("size")) {
                urlConn.setRequestProperty("Content-Length", (String) params.get("size"));
            }
...
//            br = new BufferedReader(new InputStreamReader((urlConn.getErrorStream())));
            ...
            br.close();
            urlConn.disconnect();
            JSONObject objResult = new JSONObject(response.get(0));
            if (objResult.has("error_code") && objResult.getInt("error_code") == 0 && objResult.get("data") != null) {
                JSONObject objData = (JSONObject) objResult.get("data");
                String keyEncrypted = objData.getString("k");
                String dataEncrypted = objData.getString("data");
                String dataDecrypted = AsymmetricCryptography.getInstance().DecryptTextData(keyEncrypted, dataEncrypted, Configuration.privateKeyAgent);
