...
public class FileController extends Controller {
    ...
    public Route addFile() {
        return (Request request, Response response) -> {
            ...
            try {
                ...
                fileInfo.appKey = ServletUtil.getStringParameter(request, "appKey", "");
                ...
                fileInfo.fileId = getNextValueResponse.value;

                //Find the reference FileInfo with the same checksum
                GetRefFileInfoResponse getRefFileInfoResponse = FilesCollection.getInstance().findExistedRefFileInfo(fileInfo);
	...
	@RouteInfo(method = "get,post", path = "/deleteFiles")
    public Route deleteFiles() {
        return (Request request, Response response) -> {
            response.header("Content-Type", "application/json");
            try {
				...
				for (Long fileId : fileIds)
					FilesCollection.getInstance().deleteFile(fileId);
				return new DataResponse(TErrorCode.EC_OK.getValue(), "");
            } catch (Exception ex) {
                ExceptionsCollection.getInstance().addException(
                        Configuration.SERVICE_NAME,
                        ex.getStackTrace()[0].toString(),
